...
interface MainGameProps {
  ...
  setActiveTooltip: Dispatch<SetStateAction<{ type: string; gameId: number } | null>>;
  userPlayLater: string[];
  userLikes: string[];
  userDislikes: string[];
  handlePlayLaterToggle: (gameId: string) => void;
  handleLikeToggle: (gameId: string) => void;
  handleDislikeToggle: (gameId: string) => void;
}
                ...
                <div className="md:flex-col items-center">
                  {gameDetails.genres?.length ? (
                    gameDetails.genres.map((genre) => (
                      <Badge key={genre.id} variant="default" className="mx-1">
                        {genre.name}
                      </Badge>
                    ))
                  ) : (
              ...
              <div className="mt-4">
                ...
                <p>
                  {gameDetails.developers?.map((d) => d.name).join(", ") ||
                     <span className="text-gray-500"> Developer information not available </span>}
                </p>
              </div>
                ...
                <div className="md:flex-col items-center">
                  {gameDetails.platforms?.length ? (
                    gameDetails.platforms?.map((p) => (
                      <Badge key={p.platform.id} variant="default" className="m-1">
                        {p.platform.name}
                      </Badge>
                    ))
                  ) : (
