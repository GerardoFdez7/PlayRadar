...
const Footer: React.FC<FooterProps> = ({ className = "", divClassName = ""}) => {
  return (
    <footer className={`text-center mt-12 ${className}`}>
      <div className={`relative h-[2px] bg-gray-400 dark:bg-gray-500 rounded-full before:content-[''] before:absolute before:left-1/2 before:-translate-x-1/2 before:w-[10vw] before:h-full before:bg-inherit before:rounded-full ${divClassName}`}/>
